...
namespace ImGui
{
	...
	inline bool Hotkey(const char* label, big::hotkey& hotkey_data, const ImVec2& size_arg = {})
	{
		...
		bool focus_requested = g.ActiveId == id;
		...
		auto key           = hotkey_data.get_vk_value();
		...
		if (hotkey_data.get_vk_value() != 0 && g.ActiveId != id)
		{
			strcpy_s(buf_display, key_names[hotkey_data.get_vk_value()]);
		}
		...
		if (value_changed)
		{
			big::hotkey::save_hotkeys_if_needed();
		}
